__kernel void sobel(__read_only image2d_t inputImage, __write_only image2d_t outputImage, int width, int height)
{
    ...
    for(int j=-1; j<=1; j++) {
        for(int i=-1; i<=1; i++) {
            ...
            float4 pixel = read_imagef(inputImage, CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_LINEAR, coord_offset);
    ...
    float4 gradient = (float4)(sqrt(sum.x * sum.x + sum.y * sum.y + sum.z * sum.z), 0.0f, 0.0f, 0.0f);
