...
int main(int argc, char* argv[]) {
    ...
    if (channels != 3) {
        printf("Input image must have 3 channels (RGB).\n");
        stbi_image_free(input_image);
        return -1;
    }
    unsigned char* output_image = malloc(width*height*sizeof(unsigned char));
    
    // Measure time to perform edge detection
    ...
    if (!stbi_write_png(output_filename, width, height, 1, output_image, width*sizeof(unsigned char))) {
